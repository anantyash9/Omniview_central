...
// Helper function to get a base64 data URI from an image URL via the server-side proxy
const toDataURL = (url: string): Promise<string> => new Promise((resolve, reject) => {
    // Use the new server-side proxy to get around CORS issues
    fetch(`/api/camera-proxy?url=${encodeURIComponent(url)}`)
        ...
        .catch(error => {
            console.warn(`Failed to fetch stream via proxy for ${url}. Using placeholder. Error:`, error);
            // Fallback to a placeholder data URI if the proxy fails
            resolve("data:image/png;base64,iVBORw0KGgoAAAANSUhEUgAAAYAAAAABCAYAAAA/H/aVAAAAAXNSR0IArs4c6QAAAARnQU1BAACxjwv8YQUAAAAJcEhZcwAADsMAAA7DAcdvqGQAAAAUSURBVO3BAQEAAAAABIF6f7kLqA8wAlUBASwQ08UAAAAASUVORK5CYII=");
        });
});
